...
//
// ItemDetails renders one item's full reference content — photo, canonical
// syntax (or term), description, explanation, worked examples, source-note
...
export function ItemDetails({ item, linkTarget }: { item: LearnItem; linkTarget?: '_blank' }) {
	return (
		<>
			...
			{item.syntax ? <code className="lq-command">{item.syntax}</code> : <p className="lq-term">{item.term}</p>}
