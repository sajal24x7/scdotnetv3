...
export async function GET(context) {
  ...
  // Get all posts from year collections and filter for stories
  const allPosts = await Promise.all(years.map(year => getCollection(year)));
  const flatPosts = allPosts.flat();
  const stories = flatPosts.filter(post => post.data.category === 'stories');
  
  // Sort by publish date (newest first)
  const sortedPosts = stories.sort((a, b) => 
  ...
  return rss({
    ...
    description: 'Fiction, creative writing, and narrative experiments.',
    site: context.site,
