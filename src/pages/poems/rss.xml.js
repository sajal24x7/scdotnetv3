...
export async function GET(context) {
  ...
  // Get all posts from year collections and filter for poems
  const allPosts = await Promise.all(years.map(year => getCollection(year)));
  const flatPosts = allPosts.flat();
  const poems = flatPosts.filter(post => post.data.category === 'poems');
  
  // Sort by publish date (newest first)
  const sortedPosts = poems.sort((a, b) => 
    new Date(b.data.pubDate).valueOf() - new Date(a.data.pubDate).valueOf()
  );
  
  // Generate the RSS feed
  return rss({
    title: 'Sajal Choudhary - Poems',
    description: 'Poetry, verses, and lyrical expressions from the heart.',
    site: context.site,
