...
export async function GET(context) {
  ...
  // Get all posts from year collections and filter for micro posts
  const allPosts = await Promise.all(years.map(year => getCollection(year)));
  const flatPosts = allPosts.flat();
  const microPosts = flatPosts.filter(post => post.data.category === 'micro');
  
  // Sort by publish date (newest first)
  const sortedPosts = microPosts.sort((a, b) => 
    new Date(b.data.pubDate).valueOf() - new Date(a.data.pubDate).valueOf()
  );
  
  // Generate the RSS feed
  return rss({
    title: 'Sajal Choudhary - Micro Posts',
    description: 'Short thoughts, quick observations, and micro-blogging updates.',
    site: context.site,
