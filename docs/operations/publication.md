...
- **Publish dates** — feeds and syndication trust `created` as the publish date, so bulk imports must set it honestly. The Netflix-history film/TV backfill originally stamped `created` with the import date and needed a date-cutoff filter (`isNotBackfilled`); those entries have since been rewritten to their watch dates (from the filename timestamps) and the filter removed. Any future bulk import should set `created` to the real activity date up front.
