...
**One commit per session.** Authoring a card doesn't hit the network: `stageAuthored` writes it to the `localStorage` cache and to a pending queue (`practice-authored-pending`), and the *end* of the learn session — the last card, or the Stop button — flushes the whole queue in one `POST`, which is one commit. A twelve-concept morning used to be twelve commits and twelve site rebuilds; it's now one of each. `IntroFlow` owns the boundary (`onSessionEnd`, fired exactly once per session, and awaited before quitting so navigating away can't cut the commit short).

The queue is durable and separate from the cache — the cache is "what this device knows", the queue is "what the repo hasn't seen yet". A session ended by closing the tab, going offline, or a rejected commit leaves the prompts staged, and the next session's flush picks them up. Flushing with nothing pending is a no-op that touches neither the network nor the repo, so every session can call it unconditionally.
...
A separate proposal, [`planning/mnemonic-medium-changes.md`](../../planning/mnemonic-medium-changes.md), covers the mnemonic-medium side that is still missing: rendering learn blocks inline in the notes they test, carrying context onto the back of a practice card, scaffolding prompt-writing without automating it, and making prompt revision a first-class loop. Nothing in it is implemented yet.
