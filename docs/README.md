...
| [Post Detail Layouts](components/post-layouts.md) | How `[...slug].astro` picks a detail layout per category, and what each one shows. | `src/components/layout/post/*.astro`, `src/utils/content.ts` |
...
| [Google Books API Key Setup](tools/google-books-api-key.md) | Optional key for the book cover downloader's Google Books source, avoiding the shared anonymous quota. | `GOOGLE_BOOKS_API_KEY` in `scripts/download-book-covers.js`, `.github/workflows/download-covers.yml`, `.github/workflows/enrich-shelf-metadata.yml` |
