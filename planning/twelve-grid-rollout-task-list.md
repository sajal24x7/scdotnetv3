...
- [x] **Sweep informational pages** – Convert inline grid sections on `/feeds/`, `/tags/`, `/colophon/`, and `/sajal/` to use the shared component with column span helpers, paring each route back to one grid per logical section without reintroducing wrappers.
