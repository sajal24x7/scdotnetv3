...
- [x] Audit existing layout wrappers (`LayoutContainer`, section wrappers) to confirm responsive breakpoint usage and identify conflicts with twelve-column grid introduction.
  - `LayoutContainer` now exposes a `grid` prop (`true`/`{ gap, padding, className }`) that injects the `.twelve-grid` shell and optional spacing modifiers while preserving its centering/max-width controls. Routes should lean on this toggle instead of concatenating grid classes manually so nested layouts inherit the same tokens.
  - Default to a single grid host per logical section. Reach for span helpers (`.grid-span-*`, `.grid-start-*`) before introducing nested grids; reserve additional hosts for standalone bands that need independent gutters or background treatments.
