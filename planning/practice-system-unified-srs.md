...
- Status: IN PROGRESS — Phases 0, 1, 1b, 2, 2b, and 3a done (see §7); `/practice` now exists, is the daily ritual, supports opt-in cross-device sync, and has an automated vocabulary deck
- Target pages: `/practice` (live), changes to `/learn` and `/learn/*` (live)
- Blueprint being evolved: [`docs/architecture/learning-systems.md`](../docs/architecture/learning-systems.md)
- Branch: `claude/learning-practice-system-design-9vme48` (implementation phases land on their own branches — see §7 for what's shipped)
...
**Phase 1b — FSRS adoption (§2.6). DONE.**
Swapped the engine's Leitner ladder for `ts-fsrs` (`enable_short_term: false`, `enable_fuzz: false` — day-granular, exact intervals). `CardState` is now `{ due, stability, difficulty, state, reps, lapses, lastReview }`; `gradeCard` maps "Got it"/"Forgot" to FSRS's Good/Again and calls `scheduler.next()`. `loadState` migrates a v2 blob to v3 on the fly (stability seeded from the old box interval, difficulty from FSRS's own default, `due`/`reps`/`lapses` carried over unchanged) and keeps the pre-migration blob under a `<storageKey>-v2-backup` key until `clearLegacyBackup` runs at the end of the first completed v3 session. `LearningSystem.tsx` adds a same-session requeue: a "Forgot" card gets one extra shot at the end of the session's queue (capped via a `requeued` id set) while the persisted `due` still lands on a future calendar day, never same-day — the desired-retention setting itself stays a hardcoded `DEFAULT_RETENTION = 0.9` for now, deferred to a `/practice` settings surface per §2.6. Wall-chart status thresholds updated to `learning`/`strong` via a `STRONG_STABILITY_DAYS = 21` cutoff. Verified with `astro check`, a full build, and a live Playwright run: seeded a v2 blob, confirmed the migrated streak/due state rendered immediately, ran a full session grading a due card "Forgot" then "Got it" on its same-session retry (confirmed via the doubled prompt exposure and the updated `reps`/`due` in the persisted card), confirmed the v2 backup was written then cleared after the session finished, and confirmed `/learn`'s hub picked up the resulting v3 state correctly.

**Phase 2 — `/practice` + registry. DONE.**
Added `src/data/practice-registry.ts` — a server-only `PracticeDeck[]` derived at build time from each system's existing `*-learn-config.ts` (metadata + item/prompt counts only, no dataset — safe in Astro frontmatter, never imported from a `client:load` island) — and `src/pages/api/practice/[deck].json.ts`, a prerendered per-deck dataset endpoint (`getStaticPaths` over the four deck ids, same pattern as `api/link-previews/[category].json.ts`). `engine.ts` gained the cross-deck composition: `buildUnifiedQueue` gathers each deck's own due/new candidates exactly as `buildDailySession` does (earliest-due first, capped at the deck's own `dueCap`/`newPerDay`) and merges them round-robin across decks up to new `GLOBAL_DUE_CAP = 20` / `GLOBAL_NEW_PER_DAY = 5` constants; a `roundRobinPick` helper backs both the due and new passes. Added `PracticeMeta` (the `practice-meta` key: `streak`, `lastSessionDate`, `totalSessions`, `disabledDecks`, `suspended`) with `loadPracticeMeta` (seeds `streak` from the max of existing per-deck streaks the first time it's created), `savePracticeMeta`, and `finishPracticeSession` — factored the streak-continuation math (`nextStreak`) out of `finishSession` so both the legacy per-deck streak and the new unified one share it.

`PracticeSession.tsx` (new island, `/practice.astro`) is now the only place any deck's `SrsState` gets mutated: it loads every registry deck's state from localStorage (no fetch), computes due/new counts per deck the same way `LearnHub` does, and on "Start today's practice" fetches datasets only for decks that actually contribute, builds the unified queue, and renders cards with a deck badge — reviews first, then new-item introductions, same-session "Forgot" requeue preserved from the old per-page flow. Deck toggles (`disabledDecks`), an export button (single JSON blob of every deck's storage key + `practice-meta`), and an import button (writes keys back and reloads) round out the page.

`LearningSystem.tsx` lost the `session`/`done` screens and `startDaily`/`gradeCard`/`finishSession` path entirely — it's down to `chart` (wall chart + reference panel, now linking "Go to today's practice →" to `/practice/` instead of starting a review) and `drill` (unchanged, no-op). `LearnHub.tsx` and `/learn/index.astro` now source their per-system summaries from `practiceRegistry` instead of a hand-duplicated list, and the hub gained a `Today's practice` banner (aggregate due/new across every deck, linking to `/practice/`) while each card's own status line dropped due/new/streak in favor of a `computeIntroducedCount`-derived "territory progress" percentage — since due/new grading now only happens on `/practice`, showing it on both pages would drift.

Verified with `astro check` (0 errors), a full `npm run build` (all four `/api/practice/<deck>.json` endpoints and `/practice/index.html` emitted correctly), and a live Playwright run: loaded `/practice` fresh and confirmed per-deck due/new counts matched each config's `newPerDay`/`dueCap`; ran a full session (graded one card "Forgot" then confirmed its same-session requeue fired exactly once, then "Got it" through the rest) and confirmed `practice-meta` (`streak: 1`) and all four `*-learn-srs` keys were written correctly, with the retried card's persisted `due` landing on a future date (never same-day); confirmed the global new-item cap's round-robin math against `/learn`'s territory-progress percentages (5 of 8 total new items introduced in deck order linux→finnish→til→evergreen→linux, matching `GLOBAL_NEW_PER_DAY = 5`); confirmed `/learn/linux`'s wall chart, reference panel, and drill mode still work unchanged with its CTA now pointing at `/practice/`; and confirmed deck-toggle (disabling Finnish dropped the new-item total from 8 to 5) and export (downloads a JSON blob containing exactly the keys present in localStorage) both work.

**Phase 2b — Cross-device sync (§2.8). DONE.**
Added `functions/api/practice-state.js` — `GET`/`PUT` against a `PRACTICE_STATE` KV binding, auth identical to `api/upload.js`/`api/til/sync.js` (bearer token checked against GitHub for push access to this repo; no new secret class). The blob is opaque to the function — every registry deck's `SrsState` plus `practice-meta`, keyed by localStorage key — so the function only stores and retrieves bytes; before each `PUT` it copies the previous blob to `state:backup:<UTC date>` and prunes backups older than 7 days, the safety net called for since KV has no history of its own.

`engine.ts` gained `mergeSrsState`/`mergePracticeMeta`, deterministic and idempotent per the merge rules in §2.8 (higher `reps` wins per card, tie broken by later `lastReview`; `introduced` unions with earliest date winning; `disabledDecks`/`suspended` union; `streak`/`totalSessions`/`lastSessionDate` travel together from whichever side has the later `lastSessionDate`). `PracticeSession.tsx` wires this up: a "Connect this device" input (a fine-grained PAT, stored under a dedicated `practice-sync-token` localStorage key — separate from the `/write` composer's token) triggers a pull-and-merge immediately and on every subsequent page load/tab focus; pushes fire at the end of every finished session and are debounced (4s) after each mid-session grade. Pushes read the current per-deck blobs straight from `localStorage` rather than React state, sidestepping a same-tick staleness bug where a just-graded card's `setState` update hadn't re-rendered yet. Sync failures (no binding configured, bad/expired token, offline) degrade silently — a quiet "Sync unavailable right now — practicing locally" line replaces the "last synced" status, and grading/export/import keep working with no token connected at all.

Verified without needing real Cloudflare infrastructure or a live GitHub PAT: a standalone Node harness imported `practice-state.js` directly, mocked `fetch` to stand in for the GitHub auth check (good/read-only/invalid tokens) and mocked the KV surface (`get`/`put`/`delete`/`list`) with an in-memory `Map`, and exercised every path — missing token (401), missing binding (500), read-only token (403), empty-store `GET` (`{}`), round-trip `PUT`→`GET`, a second `PUT` correctly backing up the *first* blob before overwriting, oversized-payload rejection (413), and a 7-day-old backup key getting pruned. `mergeSrsState`/`mergePracticeMeta` were exercised directly via `tsx` with hand-built local/remote fixtures covering every rule (reps tiebreak, earliest-introduced-date union, later-lastSessionDate triple, disabledDecks/suspended union) plus an idempotency check (merging a result with itself is a no-op). On the UI side, a live Playwright run against a built `astro preview` (where the Function naturally 404s, since Pages Functions don't run under plain `astro preview`) confirmed "Connect this device" persists the token, shows the quiet sync-unavailable line without any console/page error, and that grading, deck toggles, and export/import are entirely unaffected; disconnect clears the token and restores the input.

One-time setup is manual and documented (`docs/architecture/learning-systems.md`): a `PRACTICE_STATE` KV binding on the Pages project, mirroring the existing `IMAGES` R2 binding's setup flow — sync stays cleanly disabled with a clear error from the function until that binding is added.

**Phase 3a — Vocabulary feed + skip-at-introduction. DONE.**
`scripts/fetch-wotd.mjs` fetches Wiktionary's Word of the Day RSS feed (regex-based parsing against the feed's known structure — no XML/HTML-parser dependency added) and upserts each word into `src/data/vocab.generated.json`, keyed by a slugified word so re-runs are idempotent (a word already on file is never re-fetched); Merriam-Webster is wired as a second source behind `WOTD_ENABLE_MW=true`, off by default, its parser unverified against live output. `.github/workflows/fetch-wotd.yml` runs it daily and commits new words directly (not `[CI Skip]` — a new word is real new content, same reasoning as the cover-download workflow). `src/data/vocab-dataset.ts` is the pure transform (raw words → categories-by-part-of-speech + the two both-direction prompts per plan §4.2 — recognition and a gloss-cued production prompt in place of a scraped cloze sentence, since a usage example isn't reliably extractable from the feed markup); `vocab-learn-config.ts` is the thin adapter that imports the JSON and calls it, following `linux-learn-config.ts`'s "adapt at the config boundary" precedent. Registered in `practiceRegistry`, served from `/api/practice/vocab.json`, wall chart at `/learn/vocabulary`.

Skip-at-introduction (§4.4) landed generalized to every deck, not vocab alone (as this section originally scoped it): a new item's learn card on `/practice` carries "Skip — don't learn this" next to "Got it — quiz me" (`PracticeSession.tsx`); skipping adds the item id to `practice-meta.suspended` (the field and `buildUnifiedQueue`'s filtering against it already existed from Phase 2) and drops the item's learn-card-plus-prompts block from the remaining queue without touching its `SrsState`. `LearningSystem.tsx` now reads `practice-meta` read-only to render a suspended item as a distinct muted wall-chart tile (dashed, struck-through, its own legend entry) with a "Bring back to practice" un-skip action on the reference panel — no separate suspended-list page needed.

Verified with `astro check` (0 errors), a full `npm run build` (emits `/api/practice/vocab.json`, `/learn/vocabulary/index.html`, and the new-file-count reflects a seeded test blob correctly), an offline fixture test of the feed parser against a hand-built RSS sample matching Wiktionary's documented WOTD structure (the live feed itself is unreachable from this environment's network policy — the parser is untested against real Wiktionary output and should be dry-run via `workflow_dispatch` and eyeballed before trusting the schedule), `node scripts/validate-learn-data.mjs` (added a `vocab` pool), and a live Playwright run against a built `astro preview`: seeded two sample words, confirmed `/learn/vocabulary`'s wall chart renders both categories/tiles correctly; ran a `/practice` session, confirmed the very first new-item card (any deck, not just vocab) shows the Skip button, clicking it removed exactly that item's block (1 learn card + 2 prompts) from the queue and advanced correctly to the next item, and wrote the item id into `practice-meta.suspended`; confirmed the skipped item's wall-chart tile on its own `/learn/<topic>` page renders muted with the correct hover text, and that "Bring back to practice" clears the id from `suspended`.

**Phase 3b — Manual vocab capture (not started).**
`category: vocab` inbox notes with Wiktionary-REST-API definition enrichment at extraction time (plan §4.1.2). Deferred separately from 3a because it needs a new content-collection category — `CONTENT_CATEGORIES` in `src/content.config.ts`, plus whatever that implies for nav, per-category listing pages, RSS/sitemap inclusion, and backlinks — which is a content-pipeline surface change distinct from (and riskier than) the learn/practice surface every other phase has touched so far, and wants its own design pass on questions like: does a captured word get a public `/vocab/<slug>/` page, does it appear in nav, should manually captured words share `vocab.generated.json` with feed words or get their own file. A dedicated quick-add composer (§4.1.2: "a later nicety, not phase-1") stays out of scope regardless of when the note-based path lands.

**Phase 4 — People deck (local-first, §5.2). DONE.**
The ```learn``` fenced-block parser (`parseLearnBlocks`/the q/a shorthand splitter/`firstParagraph`/`firstCodeBlock`/id helpers) moved out of `scripts/extract-learn-blocks.mjs` into `src/utils/learnBlockParser.mjs` — plain JS (no TypeScript) so both the Node build script and a browser bundle can import it unmodified. `extract-learn-blocks.mjs` now imports from there instead of defining its own copy; verified byte-identical output against the pre-refactor `learn-decks.generated.json` (til/evergreen unaffected). `js-yaml` moved from `devDependencies` to `dependencies` since it's now bundled into the client, not just used by build scripts.

`src/components/learn/peopleDeckBuilder.ts` is the browser-side "Build deck file" logic: takes the `.md` notes + photo files dropped on `/learn/people`, splits frontmatter with a small hand-rolled YAML splitter (gray-matter stays build-script-only — person frontmatter is simple enough not to need it), runs bodies through the shared block parser, and produces a `LearnDataset` exactly like `extract-learn-blocks.mjs` does for TIL/evergreen — same item-id scheme (`people-<timestamp>`), same category-by-tag grouping, same newest-first `introductionOrder`. Block-less notes still work per §5.3: a default prompt (`Who is this?` → name, using the photo; or `Who fits: "<first paragraph>"?` → name without one) is generated when a note carries no learn block at all. Photos are matched against dropped image files by frontmatter `photo:` filename, downscaled via an offscreen `<canvas>` to a 128px JPEG data URI, and embedded directly on the item (`LearnItem` gained an optional `photo?: string` field, rendered as a thumbnail in `LearningSystem.tsx`'s reference panel/drill view and `PracticeSession.tsx`'s learn-card/prompt views — every other deck simply has no photo and renders nothing extra). The whole thing runs client-side; nothing is ever uploaded anywhere, and the resulting `people-deck.json` downloads directly from the browser.

`src/components/learn/peopleDeckStore.ts` holds the imported deck *content* in IndexedDB (`people-deck-db`, one record) rather than localStorage — photos push a deck well past localStorage's ~5MB budget. SRS *state* stays exactly where every other deck keeps it: `people-learn-srs` in localStorage, keyed by prompt id, via the same `loadState`/`saveState`/`gradeCard` engine functions. Re-importing (`importPeopleDeck`) replaces the IndexedDB record wholesale and prunes the SRS state's `cards`/`introduced` entries against the new item/prompt id set, per §5.2 step 4 ("items missing from a re-import are dropped and their orphaned state pruned").

`src/pages/learn/people.astro` + `PeopleLearnPage.tsx` is the page: `index={false}` (noindex, matching `/practice`'s convention), zero data shipped in the build. On mount it loads whatever's in IndexedDB (or an empty dataset) and renders the ordinary `LearningSystem` wall chart/reference panel/drills alongside two always-visible panels — "Build deck file" (file input, multiple `.md`/images, downloads the JSON) and "Load deck" (file input, one JSON, writes to IndexedDB and re-renders the chart immediately, no reload needed).

`src/data/people-learn-config.ts` holds the shared tuning constants (`newPerDay: 2`, `dueCap: 8`, `itemNoun: 'person'`, `monoAnswers: false`) so the registry entry and the live `LearnSystemConfig` can't drift. The registry entry itself (`src/data/practice-registry.ts`) is a manual `PracticeDeck` literal rather than going through `summarize()` — there's no build-time `*-learn-config.ts`/dataset to summarize, since people-note content never enters this repo (§5.1). `totalItems`/`totalPrompts` sit at 0 there; the blurb says outright that these numbers don't reflect what's actually loaded locally. `PracticeSession.tsx` closes that gap where it matters (the actual due/new math and queue composition, not just cosmetic totals): a `LOCAL_DATASET_LOADERS` map loads the people deck from IndexedDB alongside every other deck's localStorage state on mount, and `countsFor`/`totalItemsFor` use that live dataset (when present) in place of the registry's static zero — so due/new counts, the deck-list row, and `startPractice`'s queue-building are all correct for a device with a deck imported, and correctly show/contribute nothing on a device with none (per §5.4).

Verified with `astro check` (0 errors) and a full `npm run build` (emits `/learn/people/index.html` with `noindex`, and correctly emits no `/api/practice/people.json` — the deck has no server-side dataset to serve). Verified live with a scripted Playwright run against a built `astro preview` (Chromium at `/opt/pw-browsers/chromium`, real fixture notes and a generated JPEG, no manual browsing): built a deck from two fixture person notes (one with a learn block, one block-less) plus a photo, confirmed the downloaded `people-deck.json` had the correct category (grouped by the `neighbours` tag), item ids, learn-block prompts, and the block-less note's auto-generated default prompt; loaded that file back in and confirmed the wall chart rendered both tiles, the reference panel showed the embedded photo thumbnail, and the deck persisted across a full page reload (IndexedDB, not React state); then confirmed on `/practice` that the People deck showed the correct live item count and due/new counts (matching `newPerDay: 2`), appeared with its own deck badge inside a real unified session, and that grading through a full session completed normally alongside every other deck.
...
**Phase 6 — Finnish vocabulary expansion (`finnish-vocab` deck). DONE (batch 1).**
The existing Finnish deck stays exactly as rules-first as designed — untouched by this phase. `src/data/finnish-vocab.ts` is the sibling deck on the TIL-style open-feed model this section called for: 56 items across 7 themed categories (verbs 🗣️, family 👪, food 🍽️, time ⏰, question words/pronouns ❓, places 📍, adjectives 🎨), 124 prompts total. Every word gets both-direction prompts (fi→en, en→fi); 12 of the 56 additionally get a third "apply the rule" prompt (a genitive `-n` form, or a Type-1 verb conjugation for `asua`/`ostaa`) — but only where the derived form is the *regular* outcome of a consonant-gradation pattern already verified against `finnish.ts`'s own inflection bank or worked examples (each prompt's `note` cites which existing word demonstrates the same pattern — e.g. `kaupunki → kaupungin` cites `Helsinki → Helsingin`'s `nk → ng`, `äiti → äidin` and `koti → kodin` cite `katu → kadun`'s `t → d`). Per the Finnish plan's "never invent Finnish" rule, any word whose inflection wasn't independently confidence-checked this way was left as a plain two-prompt vocabulary item rather than guessed — several plausible candidates (`ruoka`, `vesi`-adjacent `uusi`, `veli`, `poika`, `lapsi`) have irregular consonant-stem declensions and were deliberately left uninflected. `introductionOrder` is flat round-robin across the 7 categories (unlike `finnish.ts`'s dependency-curated order — this deck has no rule-before-word ordering constraint to respect) so early days mix topics per blueprint principle 5.

`finnish-vocab-learn-config.ts` gives it its own tuning per this section's original call: `newPerDay: 3` (same ramp as the main Finnish deck — a flat vocab feed tolerates it), `dueCap: 14` (higher than the main deck's 12, since there's no rule-dependency backlog risk to worry about), `itemNoun: 'word'`, `monoAnswers: false`. Registered in `practiceRegistry` (`source: { kind: 'json' }`, served from `/api/practice/finnish-vocab.json`) and in `scripts/validate-learn-data.mjs`'s pool list, exactly like every other build-time deck.

Verified with `node scripts/validate-learn-data.mjs` (56 items/124 prompts, no id collisions, `introductionOrder` complete), `astro check` (0 errors), a full `npm run build` (emits `/learn/finnish-vocab/index.html` and `/api/practice/finnish-vocab.json`), and a live Playwright run against a built `astro preview`: confirmed all 56 wall-chart tiles render, spot-checked the `kaupunki` reference card's genitive example/note, drilled the "Places" category and confirmed a "Genitive (-n)" apply-rule prompt actually appears and reveals correctly, then confirmed `/practice` lists "Finnish Vocabulary · 56 words · 0 due · 3 new" and interleaves it with every other deck.

This is batch 1 (56 words, within the plan's 20–50-per-batch guidance for the low end but slightly over rather than under, to make the initial deck feel substantive) — communicative Finnish needs several more batches to reach the ~300–500-word survival threshold; adding one is purely a data-file edit (append items + category entries + extend `introductionOrder`), no architecture change.
